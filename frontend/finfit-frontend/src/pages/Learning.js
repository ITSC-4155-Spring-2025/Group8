...
function Learning() {
  ...
  const handleQuizSubmit = (answer) => {
    ...
    setQuizFeedback(answer === 'B' ? 'Correct! The 50/30/20 rule is a budgeting rule.' : 'Incorrect. The 50/30/20 rule is a budgeting rule.');
  };
    ...
    <div className="min-h-screen bg-gray-100 text-gray-900">
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6">
          ...
          <p className="mt-2 text-gray-700">{learningPassage || "Click the button to generate a learning passage."}</p>
          <button className="mt-4 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700" onClick={generateLearningContent}>
            Generate New Topic
          </button>
        </div>
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <button className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300" onClick={() => handleQuizSubmit('A')}>A) A diet plan</button>
            <button className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300" onClick={() => handleQuizSubmit('B')}>B) A budgeting rule</button>
            <button className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300" onClick={() => handleQuizSubmit('C')}>C) A savings account type</button>
          </div>
          {quizFeedback && <p className="mt-2 font-semibold text-blue-600">{quizFeedback}</p>}
        </div>

        <div className="bg-white shadow-lg rounded-2xl p-6 text-center">
          <h3 className="text-xl font-semibold">Budget Allocation Example</h3>
          <p className="text-gray-500">[Visual aid like a pie chart or infographic will go here.]</p>
        </div>

        <div className="bg-white shadow-lg rounded-2xl p-6 text-center">
          <h3 className="text-xl font-semibold">Your Progress</h3>
          <p className="text-gray-700">You've completed 3 out of 5 lessons on budgeting.</p>
        </div>
